...
% 设置推力时间偏差
manvTemp.Maneuver.Propagator.Bias = - manvDurTemp.Properties.Trip / 2;
...
manvTemp.EnableControlParameter('eVAControlManeuverFiniteBurnCenterBias');
...
CP_Bias_tauk = DC.ControlParameters.GetControlByPaths('manv' , 'FiniteMnvr.BurnCenterBias');
CP_Bias_tauk.Enable = 1;
CP_Bias_tauk.Perturbation = aux.perbDur; % s
CP_Bias_tauk.MaxStep = aux.maxStepDur; % s
...
%     satProp.ClearDWCGraphics
    ...
    % 偏移时间
    BiasConverge = manvTemp.Maneuver.Propagator.Bias;
    ...
    fprintf('STK: 推力模式: 偏移推进 \n')
    fprintf('STK: 偏移时长: %0.2f sec \n' , BiasConverge)
...
% 偏移时长
manvTemp.Maneuver.Propagator.Bias = BiasConverge;
